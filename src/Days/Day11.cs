...
namespace AdventOfCode.Days
{
    ...
    public class Day11 : BaseDay
    {
		...
		private static int CountOccupiedPartTwo(char[,] seatingMap, int[] xDiffs, int[] yDiffs, int xNum, int yNum)
		{
			int numOccupied = 0;
			foreach (var xDiff in xDiffs)
			{
				foreach (var yDiff in yDiffs)
				{
					...
					else
					{
						...
						while (seatingMap[xToCheck, yToCheck] == '.')
						{
							xToCheck -= xDiff;
							yToCheck -= yDiff;
						...
						if (seatingMap[xToCheck, yToCheck] == '#')
						{
							numOccupied++;
						}
					}
		...
		public override string PartTwo(string input)
        {
			...
			for (int i = 0; i < seatingMap.Length; i++)
			{
				...
				//Count occupied seats
				int numOccupied = CountOccupiedPartTwo(seatingMap, xDiffs, yDiffs, xNum, yNum);
